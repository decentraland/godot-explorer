        ...
        Task { [weak self] in
            ...
            do {
                ...
                if products.count == 0 && !ids.isEmpty {
                    // No products returned by StoreKit. Likely causes: (1) product still
                    // propagating in sandbox (5-30min after metadata save), (2) device not
                    // signed into Sandbox Account, (3) bundle id mismatch with ASC, (4)
                    // sandbox region != product availability region, (5) Paid Applications
                    // Agreement not signed. The UI receives an empty product list.
                    gdLog("[DclStoreKit] loadProducts: 0 products returned — check sandbox account / ASC product setup")
                }
