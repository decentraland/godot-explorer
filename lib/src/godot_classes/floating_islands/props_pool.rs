...
use godot::builtin::{Rid, Transform3D, Vector3};
...
fn grow_pool(pool: &mut PropPool, new_capacity: u32) {
    ...
    for slot in 0..pool.high_water {
        let t = rs.multimesh_instance_get_transform(pool.multimesh_rid, slot as i32);
        rs.multimesh_instance_set_transform(new_mm, slot as i32, t);
    }
