...
impl UiNode {
    ...
    // Index offset within children_container for new child entities.
    // base_control reserves slots at the START for bkg (index 0) and text
    // (index 1); the border node lives at the END so it draws on top and is
    // not counted here.
