...
/// Per-MI shadow_mesh assignment. For each visible MeshInstance3D, runs
/// `meshopt::simplify` directly on the surface arrays (target ~10% of source
/// triangles, `Sparse` flag) and builds a shadow ArrayMesh that reuses the
/// source vertex stream with the decimated index buffer. Assigned via
/// `ArrayMesh.set_shadow_mesh` so Godot rasterizes the simpler geometry in
/// the shadow pass instead of LOD0.
///
/// Skips skinned surfaces, blend-shape meshes, and surfaces below
/// `MIN_INDICES_FOR_SHADOW`. Skips MIs named `*collider*`.
pub(super) fn apply_shadow_mesh(root_node: &Gd<Node3D>) -> (u32, u32) {
    let mut lod_used = 0u32;
    let mut fallback = 0u32;
    apply_shadow_mesh_recursive(&root_node.clone().upcast(), &mut lod_used, &mut fallback);
    (lod_used, fallback)
}

fn apply_shadow_mesh_recursive(node: &Gd<Node>, lod_used: &mut u32, fallback: &mut u32) {
    let kids = node.get_children();
    for i in 0..kids.len() {
        apply_shadow_mesh_recursive(&kids.at(i), lod_used, fallback);
    }
    let Ok(mut mi) = node.clone().try_cast::<MeshInstance3D>() else {
        return;
    };
    if mi
        .get_name()
    ...
    let Some(mesh) = mi.get_mesh() else { return };
    let Ok(am) = mesh.try_cast::<ArrayMesh>() else {
        return;
    };
    if am.get_blend_shape_count() > 0 {
    ...
    // Shadow simplification target: ~10% of source triangles, error budget
    // generous enough that quadric collapse actually runs on DCL's messy
    // UV-seam topology. Same `Permissive | Sparse` flags as the visible
    // LOD chain so the simplifier doesn't refuse non-manifold input.
    //
    // Selective: only bake for surfaces with >= 6000 indices (2000 tris).
    // Smaller meshes have negligible shadow-pass cost so the meshopt
    // invocation is pure processor overhead. On Genesis Plaza this skips
    // the long tail (chairs, lamps, props) and only bakes for buildings.
    const SHADOW_TARGET_RATIO: f32 = 0.10;
    const SHADOW_TARGET_ERROR: f32 = 0.5;
    const MIN_INDICES_FOR_SHADOW: usize = 6000;

    let mut shadow_mesh = ArrayMesh::new_gd();
    ...
    for s in 0..surface_count {
        ...
        let surf_arrays = am.surface_get_arrays(s);
        let Ok(src_idx) = surf_arrays
            .at(ArrayType::INDEX.ord() as usize)
            .try_to::<PackedInt32Array>()
        else {
            continue;
        };
        if (src_idx.len() as usize) < MIN_INDICES_FOR_SHADOW {
            continue;
        }
        let Ok(src_verts) = surf_arrays
            .at(ArrayType::VERTEX.ord() as usize)
            .try_to::<PackedVector3Array>()
        else {
            continue;
        ...
        let indices_u32: Vec<u32> = src_idx.as_slice().iter().map(|&i| i as u32).collect();
        let mut vbytes: Vec<u8> = Vec::with_capacity(src_verts.len() * 12);
        for v in src_verts.as_slice() {
            vbytes.extend_from_slice(&v.x.to_le_bytes());
            vbytes.extend_from_slice(&v.y.to_le_bytes());
            vbytes.extend_from_slice(&v.z.to_le_bytes());
        }
        ...
        let target_count = ((indices_u32.len() as f32) * SHADOW_TARGET_RATIO).round() as usize;
        let target_count = target_count - (target_count % 3);
        if target_count < 3 || target_count >= indices_u32.len() {
            continue;
        }
        ...
        if simplified.is_empty()
            ...
            || (simplified.len() as f32 / indices_u32.len() as f32) > 0.9
        {
            continue;
        }

        let mut packed = PackedInt32Array::new();
        packed.resize(simplified.len());
        let slc = packed.as_mut_slice();
        for (k, &i) in simplified.iter().enumerate() {
            slc[k] = i as i32;
        }

        // Reuse the source vertex stream; only swap the index buffer.
        let mut shadow_arrays = surf_arrays.clone();
        shadow_arrays.set(ArrayType::INDEX.ord() as usize, &packed.to_variant());
        shadow_mesh.add_surface_from_arrays(primitive, &shadow_arrays);
        any_simplified = true;
    }

    if any_simplified && shadow_mesh.get_surface_count() > 0 {
        let mut am_mut = am.clone();
        am_mut.set_shadow_mesh(&shadow_mesh);
        mi.set_cast_shadows_setting(ShadowCastingSetting::ON);
        *lod_used += 1;
    } else {
        // No surface qualified — leave shadow_mesh unset; renderer uses
        // LOD0 for shadows. Counts as a fallback for reporting only.
        *fallback += 1;
    }
}
    ...
    F: FnOnce(Gd<Node3D>, &str, &SceneGltfContext) -> Result<(R, i64), anyhow::Error>,
{
    ...
    let (result, file_size) = {
        ...
        if ctx.apply_optimizations {
            let (lod_used, fallback) = apply_shadow_mesh(&node);
            if lod_used + fallback > 0 {
                godot::global::godot_print!(
                    "[shadow-mesh] from_lod={} fallback={}",
                    lod_used,
                    fallback
                );
            }
