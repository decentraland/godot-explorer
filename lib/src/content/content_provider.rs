...
/// of `resolved_optimized_base_urls()` run on tokio workers, where the singleton
/// lookup returns `None` → the override would be silently dropped and locally
/// baked content would 404 against the prod buckets. This static is updated from
...
/// Optimized-assets base URLs to try, newest first, each with no trailing slash.
/// When the CLI override is set it is tried *before* the hardcoded versioned
/// buckets, so local-dev / benchmark builds can point at a LAN preview server
/// while still falling back to prod content.
fn resolved_optimized_base_urls() -> Vec<String> {
    let mut urls = Vec::new();
    let override_url = optimized_url_override();
    if !override_url.is_empty() {
        urls.push(override_url.trim_end_matches('/').to_string());
    }
    urls.extend(
...
impl INode for ContentProvider {
    fn init(base: Base<Node>) -> Self {
        ...
        Self {
            ...
            // Default to the same URLs used for scene optimized assets (CLI
            // override first, then the versioned prod buckets).
            optimized_wearable_base_urls: Some(
                resolved_optimized_base_urls()
                    .iter()
                    .map(|url| format!("{}/", url))
                    .collect(),
            ),
    ...
    pub async fn async_fetch_optimized_asset(
        ...
        for hash_dependency in &dependencies {
            ...
            let future = async move {
                ...
                for base_url in resolved_optimized_base_urls() {
