//! Configuration for the Scene Inspector.
...
pub struct SceneInspectorConfig {
    /// Whether the Scene Inspector is enabled at runtime.
    pub enabled: bool,
    /// Directory where inspector log files are stored.
...
impl Default for SceneInspectorConfig {
    fn default() -> Self {
        ...
        let log_directory = {
            ...
            if os_name == "Android" {
                ...
                let docs_dir = os
                    ...
                    .to_string();
                PathBuf::from(docs_dir).join("DecentralandSceneInspector")
            } else {
                // Use user data directory on other platforms
                let user_dir = os.get_user_data_dir().to_string();
                PathBuf::from(user_dir).join("scene_inspector")
            }
        };
