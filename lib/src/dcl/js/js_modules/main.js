// Wasm is not allowed in scene runtimes. `--no-expose-wasm` (see `init_runtime`
// in dcl/js/mod.rs) is what actually drops the namespace; this is the fallback
// if a future V8/deno_core hands it back. Deleting rather than stubbing keeps
// `typeof WebAssembly === 'undefined'`, so feature detection still works.
delete globalThis.WebAssembly;

// Scene Inspector: wrap individual op functions to intercept all op calls.
// Active only when this scene was spawned with --scene-inspector (decided by the
// main thread and exposed to JS via op_scene_debug_enabled). Non-inspected
// scenes return immediately so Deno.core.ops keeps its native bindings and
// pays zero per-call overhead.
(function setupOpLogging() {
    const ops = Deno.core.ops;
    const debugEnabled = typeof ops.op_scene_debug_enabled === 'function'
        && ops.op_scene_debug_enabled() === true;
    if (!debugEnabled) {
        return;
    }

    const logOpStart = ops.op_scene_inspector_op_start;
    const logOpEnd = ops.op_scene_inspector_op_end;

    // Global call ID counter for correlation
    let nextCallId = 1;

    // Ops to exclude from instrumentation (to avoid infinite recursion and noise)
    const excludedOps = new Set([
        'op_scene_debug_enabled',       // Flag query op
        'op_scene_inspector_op_start',  // Don't instrument the Scene Inspector ops themselves
        'op_scene_inspector_op_end',
        'op_require',                   // Module loading
        // CRDT ops are already logged from Rust (see crdt::message::log_crdt_message
        // and engine.rs build_send_logging_ctx); wrapping them here would produce
        // duplicate entries with redundant args serialization.
