...
module.exports.getExplorerInformation = async function (body) {
    ...
    //
    // BCP-47 app locale (#2707). `configurations.locale` is what current SDKs read;
    // top-level `locale` anticipates the typed protocol field.
    const locale = Deno.core.ops.op_get_explorer_locale()
    return {
        agent: 'godot',
        platform: 'mobile',
        locale,
