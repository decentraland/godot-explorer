        ...
        match output {
            Ok(output) => {
                ...
                if installed.contains(target) {
                    print_message(MessageType::Success, &format!("{} ({})", target, description));
                } else {
                    print_message(MessageType::Info, &format!("{} ({}) - Not installed", target, description));
...
fn check_android_setup() {
    match check_android_sdk() {
        Ok(ndk_path) => {
            print_message(MessageType::Success, &format!("Android NDK found: {}", ndk_path));
...
fn check_environment_variables() {
    ...
    for (var, description) in vars_to_check {
        match env::var(var) {
            Ok(value) => {
                print_message(MessageType::Success, &format!("{}: {} ({})", var, value, description));
            }
            Err(_) => {
                print_message(MessageType::Info, &format!("{}: Not set ({})", var, description));
            }
        }
    }
}