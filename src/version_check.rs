...
/// NOT checked here: it is allocated from the Cloudflare Worker (idempotent per commit SHA) and
/// stamped into `export_presets.cfg` at export time via `DCL_BUILD_NUMBER`, so the committed
/// values are placeholders and would never match the marketing SemVer.
