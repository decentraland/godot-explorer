...
const BENCHMARK_CONFIGS: &[(u32, &str, &str)] = &[
    (
        0,
        "Single Parcel",
        "1 scene parcel with surrounding empty parcels",
    ),
    (
        2,
        "Big Scene",
        "4 parcels in cross (2x2 equivalent), typical large scene",
    ),
    (
        4,
        "Worst Case Real",
        "4 parcels spread apart, realistic worst case",
    ),
    (
        6,
        "Extreme",
        "4 parcels very spread apart, unrealistic stress test",
    ),
    (
        99,
        "Genesis Plaza",
        "Real Genesis Plaza layout (69 parcels)",
    ),
];
...
pub fn run_fi_benchmark(headless: bool) -> anyhow::Result<()> {
    ...
    print_message(
        ...
        &format!(
            ...
            if headless {
                ...
                " (with window + screenshots)"
            },
            BENCHMARK_CONFIGS
                .iter()
                .map(|(s, t, _)| format!("{}={}", t, s))
                .collect::<Vec<_>>()
        ),
    );
    ...
    for (size, title, description) in BENCHMARK_CONFIGS {
        ...
        if !status.success() {
            ...
            print_message(MessageType::Success, &format!("'{}' completed", title));
            results.push((
                *size,
                title.to_string(),
...
fn generate_csv_report(
    results: &[(u32, String, String, PathBuf)],
    output_dir: &Path,
) -> anyhow::Result<()> {
...
fn generate_markdown_report(
    ...
    writeln!(
        w,
        "This benchmark measures memory and performance impact of floating islands generation."
    )?;
    writeln!(w)?;
    writeln!(
        w,
        "**Test pattern:** 4 scene parcels arranged in a cross pattern with increasing separation."
    )?;
    writeln!(
        w,
        "This represents the worst-case scenario where empty parcels must fill large gaps."
    )?;
    writeln!(w)?;
    ...
    writeln!(
        w,
        "| Scenario | Empty Parcels | Nodes | Memory | VRAM | Gen Time |"
    )?;
    writeln!(
        w,
        "|----------|---------------|-------|--------|------|----------|"
    ...
    for (size, title, description, result_path) in results {
        ...
        if let Some(json) = parse_benchmark_result(result_path) {
            ...
            let terrain = breakdown["terrain"]
                .as_i64()
                .or_else(|| breakdown["terrain"].as_f64().map(|f| f as i64))
                .unwrap_or(0);
            let cliff = breakdown["cliff"]
                .as_i64()
                .or_else(|| breakdown["cliff"].as_f64().map(|f| f as i64))
                .unwrap_or(0);
            let grass = breakdown["grass"]
                .as_i64()
                .or_else(|| breakdown["grass"].as_f64().map(|f| f as i64))
                .unwrap_or(0);
            let tree = breakdown["tree"]
                .as_i64()
                .or_else(|| breakdown["tree"].as_f64().map(|f| f as i64))
                .unwrap_or(0);
            ...
            writeln!(
                w,
                "**Node breakdown:** {} terrain, {} cliffs, {} grass, {} trees",
                terrain, cliff, grass, tree
            )?;
            writeln!(w)?;
        }
    }
