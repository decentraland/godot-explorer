...
@export var radius: float = 1.0:
	set = set_radius
@export var fade: float = 0.05:
	set = set_fade
@export var steps: int = 32:
	set = set_steps
...
@export var auto_rotation_limit: float = 20.0:
	set = set_auto_rotation_limit
...
func _process(delta):
	...
	if auto_rotation_limit > 0:
		target_radius = (
			1.0 - (clamp(angle / auto_rotation_limit_rad, 0.0, 1.0) * (1.0 - auto_inner_radius))
		)
	...
	if auto_velocity_limit > 0:
		...
		target_radius = min(
			target_radius,
			1.0 - (clamp(velocity / auto_velocity_limit, 0.0, 1.0) * (1.0 - auto_inner_radius))
		)
