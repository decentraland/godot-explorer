...
## AVPlayer Test Scene
## This scene demonstrates how to use the AVPlayer wrapper for video playback on iOS

const AV_PLAYER_TEST_VERSION = "1.0.1"
...
func _ready():
	print("AVPlayerTest version: %s" % AV_PLAYER_TEST_VERSION)

	# Update version label
	if has_node("VBoxContainer/VersionLabel"):
		$VBoxContainer/VersionLabel.text = "v" + AV_PLAYER_TEST_VERSION

	# Create and initialize the AVPlayer
	player = AVPlayer.new()
	add_child(player)
	...
	# Show platform info
	if OS.get_name() == "iOS":
		info_label.text = "Running on iOS - AVPlayer available (v%s)" % AV_PLAYER_TEST_VERSION
	else:
		info_label.text = "Not running on iOS - AVPlayer not available (stub mode)"
