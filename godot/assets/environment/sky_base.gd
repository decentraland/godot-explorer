...
func _process(_delta:float):
	...
	sun_light.global_transform = initial_sun_transform.rotated(Vector3(1.0,0.0,0.0),PI*.49).interpolate_with(initial_sun_transform.rotated(Vector3(1.0,0.0,0.0),-PI*.49),sun_angle)
	...
	moon_light.light_energy = lerp(0.0, initial_moon_energy, smoothstep(0.0,.1,moon_angle) * smoothstep(1.0,.9,moon_angle))
	moon_light.global_transform = initial_moon_transform.rotated(Vector3(1.0,0.0,0.0),PI*.49).interpolate_with(initial_moon_transform.rotated(Vector3(1.0,0.0,0.0),-PI*.49),moon_angle)
