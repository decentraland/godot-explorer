...
func _init_gamepad_sensitivity() -> void:
	...
	var clamped_sensitivity := maxf(
		Services.config.gamepad_camera_sensitivity, MIN_GAMEPAD_CAMERA_SENSITIVITY
	)
	gamepad_camera_sensitivity.value = clamped_sensitivity
	if clamped_sensitivity != Services.config.gamepad_camera_sensitivity:
		Services.config.gamepad_camera_sensitivity = clamped_sensitivity
		Services.config.save_to_settings_file()


func _on_gamepad_camera_sensitivity_value_changed(value: float) -> void:
	Services.config.gamepad_camera_sensitivity = maxf(value, MIN_GAMEPAD_CAMERA_SENSITIVITY)
	Services.config.save_to_settings_file()
