...
func _notification(what: int) -> void:
	if what == NOTIFICATION_TRANSLATION_CHANGED and is_node_ready():
		...
		# Last: every step above can change a label's width, and the tab labels change width with
		# the language, so the scroll offset computed for the old labels can leave the active tab
		# half-cut (#2825).
