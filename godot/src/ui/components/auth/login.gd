...
func _poll_wc_connection() -> void:
	...
	match state:
		...
		"connecting":
			...
			# (can happen if callback fired while app was in background)
			if Global.is_android() and plugin.has_method("walletConnectCheckActiveSessions"):
				if plugin.walletConnectCheckActiveSessions():
					print("[WC] Recovered active session from background")
					_stop_wc_polling()
					_request_wc_signature()
					return
			# Otherwise keep polling
