...
func _ready() -> void:
	...
	Services.modal_manager.session_ended_sign_in.connect(_on_session_ended_sign_in)
	Services.modal_manager.session_ended_retry.connect(_on_session_ended_retry)
	Services.modal_manager.session_ended_exit.connect(_on_session_ended_exit)
...
func _show_disconnect_modal(reason: int) -> void:
	match reason:
		REASON_DUPLICATE_IDENTITY:
			Services.modal_manager.async_show_session_ended_modal()
		REASON_ROOM_CLOSED:
			Services.modal_manager.async_show_room_closed_modal()
		_:
			Services.modal_manager.async_show_disconnected_modal()
