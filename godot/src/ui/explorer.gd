...
func _process(_dt):
	...
	if _last_parcel_position != parcel_position:
		Services.scene_fetcher.update_position(parcel_position, false)
		...
		Services.metrics.update_position("%d,%d" % [parcel_position.x, parcel_position.y])


func get_params_from_cmd():
	var realm_string = Services.cli.realm if not Services.cli.realm.is_empty() else null
	var location_vector = Services.cli.get_location_vector()
...
func _ready():
	...
	Services.scene_runner.on_change_scene_id.connect(_on_change_scene_id)
	...
	Services.notifications_manager.notification_queued.connect(_on_notification_queued)
	...
	Services.deep_link_router.deep_link_jump.connect(_on_deep_link_jump)
	Services.deep_link_router.deep_link_open_event.connect(_on_deep_link_open_event)
	Services.deep_link_router.deep_link_open_place.connect(_on_deep_link_open_place)
	...
	elif not Services.cli.scene_inspector.is_empty():
		scene_inspector_target = Services.cli.scene_inspector
	...
	var scene_inspector_file: bool = (
		Global.deep_link_obj.scene_inspector_file or Services.cli.scene_inspector_file
	)
	if scene_inspector_file:
		Services.scene_inspector_dispatcher.set_file_logging(true)

	# Clear deep link after initial setup to prevent re-teleporting on first app resume
	Services.deep_link_router._clear_deep_link()
	...
	var start_parcel_position: Vector2i = Vector2i(Services.config.last_parcel_position)
	...
	Services.scene_runner.player_avatar_node = player.avatar
	Services.scene_runner.player_body_node = player
	Services.scene_runner.console = self._on_scene_console_message
	Services.scene_runner.pointer_tooltip_changed.connect(self._on_pointer_tooltip_changed)
	player.avatar.emote_triggered.connect(Services.scene_runner.on_primary_player_trigger_emote)
	...
	Services.scene_runner.recreate_base_ui()
	ui_safe_area.add_child(Services.scene_runner.base_ui)
	ui_safe_area.move_child(Services.scene_runner.base_ui, 0)
	...
	Services.scene_fetcher.notify_pending_loading_scenes.connect(
	...
	Services.scene_fetcher.update_position(start_parcel_position, true)

	if cmd_realm != null:
		if Realm.is_dcl_ens(cmd_realm) and Global.deep_link_obj.preview.is_empty():
			Global.async_join_world(cmd_realm)
		else:
			Services.realm.async_set_realm(cmd_realm)
			if not Global.deep_link_obj.preview.is_empty():
				Services.scene_fetcher.set_preview_url(cmd_realm)
	else:
		if Services.config.last_realm_joined.is_empty():
			Services.realm.async_set_realm(
				"https://sdk-team-cdn.decentraland.org/ipfs/goerli-plaza-main-latest"
			)
		else:
			Services.realm.async_set_realm(Services.config.last_realm_joined)
	Services.scene_runner.process_mode = Node.PROCESS_MODE_INHERIT

	Services.player_identity.logout.connect(self._on_player_logout)
	Services.player_identity.profile_changed.connect(Services.avatars.update_primary_player_profile)
	...
	if Services.avatars and Services.avatars.avatar_added:
		if not Services.avatars.avatar_added.is_connected(_on_avatar_added_apply_hide_ui):
			Services.avatars.avatar_added.connect(_on_avatar_added_apply_hide_ui)
	...
	if profile != null:
		Services.player_identity.profile_changed.emit(profile)

	Services.dcl_tokio_rpc.need_open_url.connect(self._on_need_open_url)
	Services.scene_runner.set_pause(false)
...
func _on_need_open_url(url: String, _description: String, _use_webkit: bool) -> void:
	if not Services.player_identity.get_address_str().is_empty():
		Global.open_url(url)
...
func _push_scene_interactable_area() -> void:
	if not is_instance_valid(Services.scene_runner) or not is_instance_valid(ui_safe_area):
		return
	...
	Services.scene_runner.set_interactable_area(rect)
...
func _on_player_profile_changed(_profile: DclUserProfile) -> void:
	...
	Services.notifications_manager.start_polling()
...
func _async_initialize_social_service() -> void:
	...
	if not Services.social_service.block_update_received.is_connected(_on_block_update_received):
		Services.social_service.block_update_received.connect(_on_block_update_received)
	...
	if not Services.social_service.subscription_dropped.is_connected(
		_async_on_subscription_dropped
	):
		Services.social_service.subscription_dropped.connect(_async_on_subscription_dropped)
	...
	Services.social_service.subscribe_to_block_updates()
...
func _async_fetch_blocking_status() -> void:
	var promise = Services.social_service.get_blocking_status()
	...
	if data is Dictionary:
		...
		Services.social_blacklist.init_from_blocking_status(blocked_users)


func _on_block_update_received(address: String, is_blocked: bool) -> void:
	if is_blocked:
		Services.social_blacklist.add_blocked(address)
	else:
		Services.social_blacklist.remove_blocked(address)
...
func _async_subscribe_to_friendship_updates(initial_load: bool) -> void:
	...
	while _session_active:
		var promise = Services.social_service.subscribe_to_updates()
		await PromiseUtils.async_awaiter(promise)
...
func _async_subscribe_to_connectivity_updates() -> void:
	var attempt: int = 0
	while _session_active:
		var promise = Services.social_service.subscribe_to_connectivity_updates()
		await PromiseUtils.async_awaiter(promise)
		if not _session_active:
			return
...
func _async_proactive_resubscribe() -> void:
	...
	Services.social_service.subscribe_to_connectivity_updates()
	Services.social_service.subscribe_to_block_updates()
...
func _scene_console_message(scene_id: int, level: int, timestamp: float, text: String) -> void:
	var title: String = Services.scene_runner.get_scene_title(scene_id)
	title += str(Services.scene_runner.get_scene_base_parcel(scene_id))
...
func _on_check_button_toggled(button_pressed):
	Services.scene_runner.set_pause(button_pressed)
...
func _on_panel_chat_submit_message(message: String):
	...
	if command_str.begins_with("/"):
		...
		Services.comms.send_chat(message)
		Global.on_chat_message.emit(
			Services.player_identity.get_address_str(), message, Time.get_unix_time_from_system()
		)
...
func _emit_pos_command_message() -> void:
	...
	var base_parcel: Vector2i = Services.scene_runner.get_scene_base_parcel(current_scene_id)
	...
	var realm_display: String = Services.realm.get_realm_string()
	if realm_display.is_empty():
		realm_display = Services.realm.realm_url
	var realm_type: String
	if Realm.is_genesis_city(Services.realm.realm_url):
		realm_type = "main"
	elif Realm.is_dcl_ens(realm_display) or realm_display.ends_with(".dcl.eth"):
		realm_type = "world"
	elif Realm.is_local_preview(Services.realm.realm_url):
		realm_type = "preview"
	else:
		realm_type = "realm"
...
func _on_control_menu_request_livekit_debug(enabled):
	Services.comms.set_livekit_debug(enabled)
	if enabled:
...
func _on_control_menu_request_pause_scenes(enabled):
	Services.scene_runner.set_pause(enabled)
...
func move_to(position: Vector3, skip_loading: bool, check_stuck: bool = true):
	...
	if not skip_loading:
		if not Services.scene_fetcher.is_scene_loaded(cur_parcel_position.x, cur_parcel_position.y):
			loading_ui.enable_loading_screen()
			# LOADING_START metric
			var loading_data = {
				"position": str(position),
				"realm": Services.realm.get_realm_string(),
				"when": "on_moveto"
			}
			Services.metrics.track_screen_viewed("LOADING_START", JSON.stringify(loading_data))
...
func _async_try_change_realm(realm_string: String, when: String) -> void:
	...
	var success = await Services.realm.async_set_realm(realm_string, true)
	if success:
		loading_ui.enable_loading_screen()
		var loading_data = {
...
func _async_teleport_to(parcel: Vector2i, realm: String = "") -> void:
	if not realm.is_empty() and realm != Services.realm.get_realm_string():
		var success = await Services.realm.async_set_realm(realm)
		if not success:
			return
		loading_ui.enable_loading_screen()
	...
	Services.scene_fetcher.update_position(parcel, true)

	Services.config.add_place_to_last_places(parcel, realm)
	dirty_save_position = true
...
func _on_timer_fps_label_timeout():
	...
	if Services.config.dynamic_graphics_enabled:
		var dm = Services.dynamic_graphics_manager
		if dm == null:
...
func set_cursor_position(position: Vector2):
	if Services.scene_runner.raycast_use_cursor_position:
		...
		Services.scene_runner.set_cursor_position(position)
...
func _on_button_load_scenes_pressed() -> void:
	Services.scene_fetcher._bypass_loading_check = true
...
func _is_in_preview_realm() -> bool:
	...
	if not preview_url.is_empty():
		return Services.realm.realm_string == preview_url
	return Services.cli.preview_mode
...
func _async_open_profile_by_address(user_address: String):
	var promise = Services.content_provider.fetch_profile(user_address)
...
func _async_open_profile_by_avatar(avatar: DclAvatar):
	...
	if avatar is Avatar:
		...
		if not avatar_id.is_empty():
			...
			if Services.social_blacklist.is_blocked(avatar_id):
				return
...
func _show_notification_toast(notification_d: Dictionary) -> void:
	...
	if notif_type == "social_service_friendship_request":
		...
		if not sender_address.is_empty() and Services.social_blacklist.is_blocked(sender_address):
			# Immediately dequeue this notification and try to show next one
			Services.notifications_manager.dequeue_notification()
			return
...
func _on_toast_mark_as_read(notification_d: Dictionary) -> void:
	...
	if not notification_id.is_empty():
		...
		Services.notifications_manager.mark_as_read(ids)
...
func _on_loading_finished() -> void:
	...
	if not Services.modal_manager.ban_pre_check_active:
...
func _async_run_ban_check() -> void:
	...
	var realm_name = Services.realm.get_realm_string()
	...
	if Realm.is_dcl_ens(realm_name):
		...
		var parcel = Services.scene_fetcher.current_position
	...
	if not allowed and generation == _ban_check_generation:
		Services.modal_manager.ban_pre_check_active = true
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_APPLICATION_FOCUS_IN:
		...
		Services.notifications_manager.clear_badge_and_delivered_notifications()

		# Resync notification queue to clean up fired notifications and reschedule next batch
		Services.notifications_manager.force_queue_sync()
...
func _share_place():
	...
	if is_genesis_city:
		...
		if current_scene_id == -1 and Services.scene_fetcher.loaded_scenes.size() == 1:
			var scene: SceneFetcher.SceneItem = Services.scene_fetcher.loaded_scenes.values()[0]
		...
		var realm_url = Services.realm.realm_url
...
func _on_change_scene_id(scene_id: int):
	is_genesis_city = Realm.is_genesis_city(Services.realm.realm_url)
	if scene_id == -1:
		scene_title = ""
		_update_preview_ui(false)
		return
	var scene = Services.scene_fetcher.get_scene_data_by_scene_id(scene_id)
	if scene != null:
...
func _apply_hide_ui_to_avatar_nicks(hide: bool) -> void:
	# Remote avatars
	if Services.avatars:
		var avatars = Services.avatars.get_avatars() if "get_avatars" in Services.avatars else []
		for a in avatars:
			if a is Avatar:
				(a as Avatar).set_force_hide_name(hide)
	# Local player avatar
	if Services.scene_runner and is_instance_valid(Services.scene_runner.player_avatar_node):
		var p = Services.scene_runner.player_avatar_node
		if p is Avatar:
