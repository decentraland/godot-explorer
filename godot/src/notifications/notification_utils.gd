...
## Normalizes accented characters to prevent display issues while preserving emojis
## Note: If emojis still show as symbols, they will need to be removed separately
## @param text: The text to sanitize
## @param remove_emojis: If true, removes emojis. If false, preserves them (default: false)
## @return: Sanitized text with normalized accents, optionally without emojis
static func sanitize_notification_text(text: String, remove_emojis: bool = false) -> String:
	if text.is_empty():
		return text
	...
	# This helps prevent display issues with accented characters on iOS
	var accent_map = {
		"á": "a",
		"à": "a",
		"ä": "a",
	...
	# Return text with normalized accents but emojis preserved
	return sanitized
