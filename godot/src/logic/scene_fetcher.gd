...
# Nearby scene notification state
var _home_scene_entity_id: String = ""
var _home_scene_parcels: Array[Vector2i] = []
var _pending_toast_parcel: Vector2i = INVALID_PARCEL
...
var _scene_toast_cache: Dictionary = {}  # entity_id -> place Dictionary
...
func _ready():
	...
	_scene_toast_timer.timeout.connect(self._async_on_scene_toast_timer_timeout)
...
func update_position(new_position: Vector2i, is_teleport: bool) -> void:
	...
	if is_teleport:
		...
		_home_scene_parcels = []
		_pending_toast_parcel = INVALID_PARCEL
...
func _async_on_scene_toast_timer_timeout() -> void:
	_pending_toast_parcel = INVALID_PARCEL
	if _home_scene_entity_id.is_empty() or current_position in _home_scene_parcels:
		return
	await _async_show_scene_toast(current_position)
...
func _async_check_nearby_scene(new_position: Vector2i) -> void:
	...
	if _home_scene_entity_id.is_empty() and not loaded_entity_id.is_empty():
		...
		var home_data: SceneItem = loaded_scenes.get(_home_scene_entity_id)
		if home_data != null:
	...
	if _scene_toast_timer.time_left > 0:
		...
		if _pending_toast_entity_id.is_empty():
			var resolved := scene_entity_coordinator.get_scene_entity_id(_pending_toast_parcel)
			if not resolved.is_empty():
				_pending_toast_entity_id = resolved
		if _pending_toast_scene_parcels.is_empty() and not _pending_toast_entity_id.is_empty():
			var scene_data: SceneItem = loaded_scenes.get(_pending_toast_entity_id)
			if scene_data != null:
	...
	if not new_entity_id.is_empty():
		var scene_data: SceneItem = loaded_scenes.get(new_entity_id)
		if scene_data != null:
			_pending_toast_scene_parcels = scene_data.parcels.duplicate()
...
func _async_show_scene_toast(parcel: Vector2i) -> void:
	var entity_id := scene_entity_coordinator.get_scene_entity_id(parcel)
