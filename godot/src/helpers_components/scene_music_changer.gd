...
func _on_change_scene_id(scene_id: int):
	if scene_id == -1:  # Empty parcel
		Services.music_player.play(_RANDOM_MUSICS.pick_random())
	else:
		var current_music = Services.music_player.get_current_music()
		if _RANDOM_MUSICS.has(current_music):
			Services.music_player.stop()  # We dont play music on an empty parcel
