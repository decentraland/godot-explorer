...
static func apply_general_volume_settings():
	...
	var general_db = -80.0 + (80.0 * (float(Services.config.audio_general_volume) / 100.0))
	AudioServer.set_bus_volume_db(bus_index, general_db)


static func apply_scene_volume_settings(force_value = null):
	var scene_volume: float = Services.config.audio_scene_volume
...
static func apply_voice_chat_volume_settings(force_value = null):
	var voice_volume: float = Services.config.audio_voice_chat_volume
	if force_value is float:
...
static func apply_ui_volume_settings():
	...
	var general_db = -80.0 + (80.0 * (float(Services.config.audio_ui_volume) / 100.0))
	AudioServer.set_bus_volume_db(bus_index, general_db)


static func apply_music_volume_settings():
	var bus_index := AudioServer.get_bus_index("Music")
	var general_db = -80.0 + (80.0 * (float(Services.config.audio_music_volume) / 100.0))
	AudioServer.set_bus_volume_db(bus_index, general_db)
...
static func apply_avatar_and_emotes_volume_settings():
	...
	var general_db = (
		-80.0 + (80.0 * (float(Services.config.audio_avatar_and_emotes_volume) / 100.0))
	)
	AudioServer.set_bus_volume_db(bus_index, general_db)
